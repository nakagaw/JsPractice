module.exports = function(grunt) {
  grunt.initConfig({
    ...
    bower: { //バウアーからリストの項目を_libディレクトリにコピー
      install: {
        options: {
          targetDir: '_lib/',
          install: true,
    ...
    clean: { // _lib以下の不要ファイルを削除して残りをreleaseディレクトリに
      init: ["_lib/"],
      build: ["<%= dir.release %>"]
    },
    sass: { //_sassをコンパイル
    ...
    connect: { //devディレクトリをローカルサーバー立てて表示
      server: {
        options: {
          base: '<%= dir.dev %>'
        }
    ...
    watch: { // html、sass、coffeeを監視&コンパイル
      options: {
        livereload: true
      },
      html: {
  ...
  return grunt.registerTask("release", ['uglify', 'clean:build', 'copy:build']);
};
